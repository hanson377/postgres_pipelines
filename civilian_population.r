...
civilian_population <- blsAPI(payload, api_version = 2, return_data_frame = T)
}
...
civilian_population <- rbindlist(data)

civilian_population$area_code <- substr(civilian_population$seriesID,4,18)
civilian_population <- civilian_population %>% left_join(state_keys, by='area_code')
civilian_population <- civilian_population %>% select(area_code,area_text,year,period,periodname=periodName,value) %>% mutate(geo_type = 'state')


## drop old table, create new one
dbExecute(con,"DROP TABLE civilian_population;")
dbWriteTable(con, "civilian_population", civilian_population, OVERWRITE = TRUE, append = TRUE,row.names =FALSE)

## test that data is live
data_test <- dbGetQuery(con, "SELECT * FROM civilian_population")
rm(data_test)
