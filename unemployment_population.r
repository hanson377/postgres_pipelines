...
unemployment_population <- blsAPI(payload, api_version = 2, return_data_frame = T)
}
...
unemployment_population <- rbindlist(data)

unemployment_population$area_code <- substr(unemployment_population$seriesID,4,18)
unemployment_population <- unemployment_population %>% left_join(state_keys, by='area_code')
unemployment_population <- unemployment_population %>% select(area_code,area_text,year,period,periodname=periodName,value) %>% mutate(geo_type = 'state')


## drop old table, create new one
dbExecute(con,"DROP TABLE unemployment_population;")
dbWriteTable(con, "unemployment_population", unemployment_population, OVERWRITE = TRUE, append = TRUE,row.names =FALSE)

## test that data is live
data_test <- dbGetQuery(con, "SELECT * FROM unemployment_population")
rm(data_test)
