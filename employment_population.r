...
employment_population <- blsAPI(payload, api_version = 2, return_data_frame = T)
}
...
employment_population <- rbindlist(data)

employment_population$area_code <- substr(employment_population$seriesID,4,18)
employment_population <- employment_population %>% left_join(state_keys, by='area_code')
employment_population <- employment_population %>% select(area_code,area_text,year,period,periodname=periodName,value) %>% mutate(geo_type = 'state')


## drop old table, create new one
dbExecute(con,"DROP TABLE employment_population")
dbWriteTable(con, "employment_population", employment_population, OVERWRITE = TRUE, append = TRUE,row.names =FALSE)

## test that data is live
data_test <- dbGetQuery(con, "SELECT * FROM employment_population")
rm(data_test)
